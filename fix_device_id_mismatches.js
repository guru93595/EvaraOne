...
// Initialize Firebase
const serviceAccount = require('./backend/src/config/firebase-secure.json');
admin.initializeApp({
...
async function run() {
  try {
    ...
    console.log(`   With metadata: ${mismatches.filter(m => m.hasMetadata).length}`);
    console.log(`   Orphaned: ${mismatches.filter(m => !m.hasMetadata).length}`);
    ...
    console.log(`If using caching, flush cache:/`);
    console.log(`   redis-cli FLUSHDB  (development)`);
    console.log(`   Or restart backend to clear in-memory cache\n`);
